...
function taskGraphFactory(
    ...
    TaskGraph.prototype._topoSortVisit = function(taskId, parentTaskName, sorted, layer) {
        var self = this;
        ...
        var depth = _.max(_.map(task.waitingOn, function(value, dep) {
            ...
            if (value === 'finished' && _.contains(Constants.FinishedTaskStates, value)) {
                ...
                self.tasks[dep].nonTerminalOnStates = Array.isArray(value) ? value : [value];
            }
            return self._topoSortVisit(dep, task.injectableName, sorted, layer + 1);
        }));
    ...
    TaskGraph.prototype.detectCyclesAndMarkTerminalTasks = function() {
        var self = this;
        ...
        _.forEach(self.tasks, function(task) {
            ...
            // a waitOn dependency for a failed state is found, that state will be
            // removed from the terminalOnStates array.
            task.terminalOnStates = _.difference(
                Constants.FailedTaskStates, task.nonTerminalOnStates
            );
            delete task.nonTerminalOnStates;
        });
        _.forEach(_.last(sorted), function(taskLabel) {
            ...
            self.markBranchedTerminalTasks(self.tasks[taskLabel].waitingOn);
        });
    };

    TaskGraph.prototype.markBranchedTerminalTasks = function(waitingOn) {
        var self = this;

        _.forEach(waitingOn, function(states, task) {
            // Works for both array and string types of <states>
            if (!_.contains(states, Constants.TaskStates.Succeeded)) {
                ...
                self.tasks[task].terminalOnStates = _.union(
                    self.tasks[task].terminalOnStates, [Constants.TaskStates.Succeeded]);
                return self.markBranchedTerminalTasks(self.tasks[task].waitingOn);
            }
        });
    };
