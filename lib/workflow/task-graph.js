...
function taskGraphFactory(
    ...
    TaskGraph.prototype._setTerminalStates = function(task, nonTerminalOnStates) {
        // All tasks are terminal on failed states by default. If a task with
        // a waitOn dependency for the current task's failed state is found,
        // that state will be removed from its terminalOnStates array.
        // If the task has already been visited, preserve existing
        // terminalOnStates information.
        if (_.isEmpty(task.terminalOnStates)) {
            task.terminalOnStates = _.difference(
                    Constants.FailedTaskStates, nonTerminalOnStates);
        } else {
            task.terminalOnStates = _.difference(
                    task.terminalOnStates, nonTerminalOnStates);
        }
        if (!_.isEmpty(nonTerminalOnStates) &&
            !_.contains(nonTerminalOnStates, Constants.TaskStates.Succeeded) &&
            !_.contains(task.terminalOnStates, Constants.TaskStates.Succeeded)) {
            // If a task is only dependent on a failure state, it means that a succeeded
            // state of the dependent task is potentially terminal for the graph.
            task.terminalOnStates = _.union(
                task.terminalOnStates, [Constants.TaskStates.Succeeded]);
        }
    };

    TaskGraph.prototype._topoSortVisit = function(
            taskId, parentTaskName, sorted, layer, markers, nonTerminalOnStates) {
        var self = this;
        ...
        var depth = _.max(_.map(task.waitingOn, function(value, dep) {
            ...
            if (value === 'finished' && _.contains(Constants.FinishedTaskStates, value)) {
                return 0;
            } else if (self.tasks[dep].state === value) {
                return 0;
            } else {
                // Expand 'finished' to all finished task states
                if (value === Constants.TaskStates.Finished) {
                    value = Constants.FinishedTaskStates;
                    task.waitingOn[dep] = Constants.FinishedTaskStates;
                }
            ...
            return self._topoSortVisit(
                dep, task.injectableName, sorted, layer + 1, markers, nonTerminalOnStates);
        }));
        ...
        marker.layer = depth;
        if (sorted.length - 1 < marker.layer) {
            sorted.push([]);
        }
        sorted[marker.layer].push(taskId);
        // Tell the dependant task that it is a layer above this one in the sort.
        return marker.layer + 1;
    };
    ...
    TaskGraph.prototype.topologicalSortTasks = function() {
        var self = this;
        var sorted = [];
        var markers = {};
        _.forEach(self.tasks, function(task, taskId) {
            self._topoSortVisit(taskId, null, sorted, 0, markers, null);
        });
        return sorted;
    };
    ...
    TaskGraph.prototype.detectCyclesAndSetTerminalTasks = function() {
        var self = this;
        // This will throw if a cycle is detected
        var sorted = this.topologicalSortTasks();
        // Mark all tasks that are last in the graph execution order as
        // potentially terminal on success.
        _.forEach(_.last(sorted), function(taskLabel) {
            assert.ok(_.has(self.tasks, taskLabel));
            self.tasks[taskLabel].terminalOnStates.push(Constants.TaskStates.Succeeded);
        });
    };
    ...
    TaskGraph.prototype.validate = function () {
        ...
        .spread(function() {
            self.detectCyclesAndMarkTerminalTasks();
        })
        .then(function() {
            return self;
        });
