...
function taskGraphFactory(
    ...
    TaskGraph.prototype._visitGraphNode = function(
        ...
        // Only check for null here, not for an empty array!
        if (!task.terminalOnStates) {
            task.terminalOnStates = _.difference(Constants.FinishedTaskStates, nonTerminalOnStates);
        } else {
            task.terminalOnStates = _.difference(task.terminalOnStates, nonTerminalOnStates);
        }
