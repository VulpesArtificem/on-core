...
function ModelFactory (
    ...
    return Waterline.Collection.extend({
        ...
        addWaterlineDateFieldsMongo: function(obj) {
            obj.$set = obj.$set || {};
            obj.$setOnInsert = obj.$setOnInsert || {};
            if (!obj.$set.updatedAt) {
                obj.$set.updatedAt = new Date();
            }
            if (!obj.$setOnInsert.createdAt) {
                obj.$setOnInsert = new Date();
            }
        },
        ...
        runNativeMongo: function(func, args) {
            ...
            .then(function(collection) {
                // node mongo driver resolves bluebird promises when a callback is not passed
                return collection[func].apply(collection, args);
            });
        },

        findMongo: function(query) {
        ...
        findAndModifyMongo: function() {
            ...
            this.addWaterlineDateFieldsMongo(update);
            return this.runNative('findAndModify', Array.prototype.slice.call(arguments));
        },

        updateManyMongo: function(filter, update, options) {
            this.addWaterlineDateFieldsMongo(update);
            return this.runNative('updateMany', [filter, update, options]);
        },
